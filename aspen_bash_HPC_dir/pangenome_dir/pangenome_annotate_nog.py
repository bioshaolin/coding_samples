...
db = "/scicomp/home/opw9/hmm_universal_dir/nog_dir/all_nog.hmm" # this is the combined file of all the HMM profiles.
...
for protein_file in os.listdir(input_folder):
	...
	if protein_file.endswith(".faa"):
		...
		output_path = re.sub(".faa", ".nog.hmmout", input_path)
		print(protein_file, input_path, output_path)

		cmd = "hmmsearch --cut_nc --tblout " + output_path + " " + db + " " + input_path
		print(cmd)
		cmd2 = shlex.split(cmd)
		subprocess.call(cmd2, stdout=open('hmmout.nog.txt', 'a'), stderr=open("error_file.txt", 'a'))

dirs = os.listdir(input_folder)
for filenames in dirs:
	if filenames.endswith(".nog.hmmout"):

		acc = re.sub(".nog.hmmout", "", filenames)
