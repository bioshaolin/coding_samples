...
(Genbank = g, Refseq = r, continue = c)  " choice
	if [ "$choice" == "g" ] ; then
		...
		if [ "$choice2" == $choice2 ] ; then
			curl -o archaea_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/archaea/
			awk '{print $9}' archaea_ftp_temp_list.html > archaea_ftp_list_temp_1.txt
			grep -n $choice2 archaea_ftp_list_temp_1.txt > archaea_ftp_list_temp_2.txt
			cat archaea_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > archaea_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' archaea_ftp_list_temp_3.txt > archaea_ftp_list_temp_4.txt
			cat archaea_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/archaea/;g' > archaea_ftp_list_temp_5.txt
			cat archaea_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > archaea_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat archaea_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > archaea_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat archaea_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt archaea_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > archaea_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "r" ] ; then
		read -p "What Organism?  " choice2
		if [ "$choice2" == $choice2 ] ; then
			curl -o archaea_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/archaea/
			awk '{print $9}' archaea_ftp_temp_list.html > archaea_ftp_list_temp_1.txt
			grep -n $choice2 archaea_ftp_list_temp_1.txt > archaea_ftp_list_temp_2.txt
			cat archaea_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > archaea_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' archaea_ftp_list_temp_3.txt > archaea_ftp_list_temp_4.txt
			cat archaea_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/archaea/;g' > archaea_ftp_list_temp_5.txt
			cat archaea_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary_historical.txt;g' > archaea_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat archaea_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > archaea_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat archaea_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt archaea_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > archaea_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "c" ] ; then
	...
	if grep -q "$choice2" archaea_ftp_list_$choice."$choice2".$(date +%m.%d.%Y%p)_3.txt ; then
...
rm -f archaea_ftp_temp_list.html
rm -f archaea_ftp_list_temp_1.txt
rm -f archaea_ftp_list_temp_2.txt
rm -f archaea_ftp_list_temp_3.txt
rm -f archaea_ftp_list_temp_4.txt
rm -f archaea_ftp_list_temp_5.txt
rm -f archaea_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
rm -f archaea_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt

cat *archaea_ftp* > arc_ftp_list.txt
cat arc_ftp_list.txt | while read line
do
	wget $line
done
...
if [ "$proj" == $proj ] ; then
	...
	mv -f *archaea_ftp_list* ~/$proj
	mv -f arc_ftp_list.txt ~/$proj
	cd $proj
