...
(Genbank = g, Refseq = r, continue = c)  " choice
	if [ "$choice" == "g" ] ; then
		...
		if [ "$choice2" == $choice2 ] ; then
			curl -o protozoa_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/protozoa/
			awk '{print $9}' protozoa_ftp_temp_list.html > protozoa_ftp_list_temp_1.txt
			grep -n $choice2 protozoa_ftp_list_temp_1.txt > protozoa_ftp_list_temp_2.txt
			cat protozoa_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > protozoa_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' protozoa_ftp_list_temp_3.txt > protozoa_ftp_list_temp_4.txt
			cat protozoa_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/protozoa/;g' > protozoa_ftp_list_temp_5.txt
			cat protozoa_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > protozoa_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat protozoa_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > protozoa_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat protozoa_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt protozoa_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > protozoa_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "r" ] ; then
		read -p "What Organism?  " choice2
		if [ "$choice2" == $choice2 ] ; then
			curl -o protozoa_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/protozoa/
			awk '{print $9}' protozoa_ftp_temp_list.html > protozoa_ftp_list_temp_1.txt
			grep -n $choice2 protozoa_ftp_list_temp_1.txt > protozoa_ftp_list_temp_2.txt
			cat protozoa_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > protozoa_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' protozoa_ftp_list_temp_3.txt > protozoa_ftp_list_temp_4.txt
			cat protozoa_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/protozoa/;g' > protozoa_ftp_list_temp_5.txt
			cat protozoa_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary_historical.txt;g' > protozoa_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat protozoa_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > protozoa_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat protozoa_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt protozoa_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > protozoa_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "c" ] ; then
	...
	if grep -q "$choice2" protozoa_ftp_list_$choice."$choice2".$(date +%m.%d.%Y%p)_3.txt ; then
...
rm -f protozoa_ftp_temp_list.html
rm -f protozoa_ftp_list_temp_1.txt
rm -f protozoa_ftp_list_temp_2.txt
rm -f protozoa_ftp_list_temp_3.txt
rm -f protozoa_ftp_list_temp_4.txt
rm -f protozoa_ftp_list_temp_5.txt
rm -f protozoa_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
rm -f protozoa_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt

cat *protozoa_ftp* > proto_ftp_list.txt
cat proto_ftp_list.txt | while read line
do
	wget $line
done
...
if [ "$proj" == $proj ] ; then
	...
	mv -f *protozoa_ftp_list* ~/$proj
	mv -f proto_ftp_list.txt ~/$proj
	cd $proj
	...
	sort -u ftp_genome_list_temp.txt > ftp_genome_list.txt
	...
	sort -u ftp_gbff_list_temp.txt > ftp_gbff_list.txt
	...
	sort -u ftp_gff_list_temp.txt > ftp_gff_list.txt
	...
	sort -u ftp_rna_from_genomic_list_temp.txt > ftp_rna_from_genomic_list.txt
