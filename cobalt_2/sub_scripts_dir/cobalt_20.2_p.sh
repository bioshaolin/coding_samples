...
(Genbank = g, Refseq = r, continue = c)  " choice
	if [ "$choice" == "g" ] ; then
		...
		if [ "$choice2" == $choice2 ] ; then
			curl -o plant_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/plant/
			awk '{print $9}' plant_ftp_temp_list.html > plant_ftp_list_temp_1.txt
			grep -n $choice2 plant_ftp_list_temp_1.txt > plant_ftp_list_temp_2.txt
			cat plant_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > plant_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' plant_ftp_list_temp_3.txt > plant_ftp_list_temp_4.txt
			cat plant_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/plant/;g' > plant_ftp_list_temp_5.txt
			cat plant_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > plant_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat plant_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > plant_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat plant_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt plant_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > plant_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "r" ] ; then
		read -p "What Organism?  " choice2
		if [ "$choice2" == $choice2 ] ; then
			curl -o plant_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/plant/
			awk '{print $9}' plant_ftp_temp_list.html > plant_ftp_list_temp_1.txt
			grep -n $choice2 plant_ftp_list_temp_1.txt > plant_ftp_list_temp_2.txt
			cat plant_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > plant_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' plant_ftp_list_temp_3.txt > plant_ftp_list_temp_4.txt
			cat plant_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/plant/;g' > plant_ftp_list_temp_5.txt
			cat plant_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary_historical.txt;g' > plant_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat plant_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > plant_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat plant_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt plant_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > plant_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "c" ] ; then
	...
	if grep -q "$choice2" plant_ftp_list_$choice."$choice2".$(date +%m.%d.%Y%p)_3.txt ; then
...
rm -f plant_ftp_temp_list.html
rm -f plant_ftp_list_temp_1.txt
rm -f plant_ftp_list_temp_2.txt
rm -f plant_ftp_list_temp_3.txt
rm -f plant_ftp_list_temp_4.txt
rm -f plant_ftp_list_temp_5.txt
rm -f plant_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
rm -f plant_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt

cat *plant_ftp* > plant_ftp_list.txt
cat plant_ftp_list.txt | while read line
do
	wget $line
done
...
if [ "$proj" == $proj ] ; then
	...
	mv -f *plant_ftp_list* ~/$proj
	mv -f plant_ftp_list.txt ~/$proj
	cd $proj
