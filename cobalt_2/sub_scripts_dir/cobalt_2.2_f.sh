...
(Genbank = g, Refseq = r, continue = c)  " choice
	if [ "$choice" == "g" ] ; then
		...
		if [ "$choice2" == $choice2 ] ; then
			curl -o fungi_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/fungi/
			awk '{print $9}' fungi_ftp_temp_list.html > fungi_ftp_list_temp_1.txt
			grep -n $choice2 fungi_ftp_list_temp_1.txt > fungi_ftp_list_temp_2.txt
			cat fungi_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > fungi_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' fungi_ftp_list_temp_3.txt > fungi_ftp_list_temp_4.txt
			cat fungi_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/fungi/;g' > fungi_ftp_list_temp_5.txt
			cat fungi_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > fungi_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat fungi_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > fungi_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat fungi_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt fungi_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > fungi_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "r" ] ; then
		read -p "What Organism?  " choice2
		if [ "$choice2" == $choice2 ] ; then
			curl -o fungi_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/fungi/
			awk '{print $9}' fungi_ftp_temp_list.html > fungi_ftp_list_temp_1.txt
			grep -n $choice2 fungi_ftp_list_temp_1.txt > fungi_ftp_list_temp_2.txt
			cat fungi_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > fungi_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' fungi_ftp_list_temp_3.txt > fungi_ftp_list_temp_4.txt
			cat fungi_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/fungi/;g' > fungi_ftp_list_temp_5.txt
			cat fungi_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary_historical.txt;g' > fungi_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat fungi_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > fungi_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat fungi_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt fungi_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > fungi_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "c" ] ; then
	...
	if grep -q "$choice2" fungi_ftp_list_$choice."$choice2".$(date +%m.%d.%Y%p)_3.txt ; then
...
rm -f fungi_ftp_temp_list.html
rm -f fungi_ftp_list_temp_1.txt
rm -f fungi_ftp_list_temp_2.txt
rm -f fungi_ftp_list_temp_3.txt
rm -f fungi_ftp_list_temp_4.txt
rm -f fungi_ftp_list_temp_5.txt
rm -f fungi_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
rm -f fungi_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt

cat *fungi_ftp* > fungi_ftp_list.txt
cat fungi_ftp_list.txt | while read line
do
	wget $line
done
...
if [ "$proj" == $proj ] ; then
	...
	mv -f *fungi_ftp_list* ~/$proj
	mv -f fungi_ftp_list.txt ~/$proj
	cd $proj
