...
(Genbank = g, Refseq = r, continue = c)  " choice
	if [ "$choice" == "g" ] ; then
		...
		if [ "$choice2" == $choice2 ] ; then
			curl -o invertebrate_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/invertebrate/
			awk '{print $9}' invertebrate_ftp_temp_list.html > invertebrate_ftp_list_temp_1.txt
			grep -n $choice2 invertebrate_ftp_list_temp_1.txt > invertebrate_ftp_list_temp_2.txt
			cat invertebrate_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > invertebrate_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' invertebrate_ftp_list_temp_3.txt > invertebrate_ftp_list_temp_4.txt
			cat invertebrate_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/invertebrate/;g' > invertebrate_ftp_list_temp_5.txt
			cat invertebrate_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > invertebrate_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat invertebrate_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > invertebrate_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat invertebrate_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt invertebrate_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > invertebrate_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "r" ] ; then
		read -p "What Organism?  " choice2
		if [ "$choice2" == $choice2 ] ; then
			curl -o invertebrate_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/invertebrate/
			awk '{print $9}' invertebrate_ftp_temp_list.html > invertebrate_ftp_list_temp_1.txt
			grep -n $choice2 invertebrate_ftp_list_temp_1.txt > invertebrate_ftp_list_temp_2.txt
			cat invertebrate_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > invertebrate_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' invertebrate_ftp_list_temp_3.txt > invertebrate_ftp_list_temp_4.txt
			cat invertebrate_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/invertebrate/;g' > invertebrate_ftp_list_temp_5.txt
			cat invertebrate_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary_historical.txt;g' > invertebrate_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat invertebrate_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > invertebrate_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat invertebrate_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt invertebrate_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > invertebrate_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "c" ] ; then
	...
	if grep -q "$choice2" invertebrate_ftp_list_$choice."$choice2".$(date +%m.%d.%Y%p)_3.txt ; then
...
rm -f invertebrate_ftp_temp_list.html
rm -f invertebrate_ftp_list_temp_1.txt
rm -f invertebrate_ftp_list_temp_2.txt
rm -f invertebrate_ftp_list_temp_3.txt
rm -f invertebrate_ftp_list_temp_4.txt
rm -f invertebrate_ftp_list_temp_5.txt
rm -f invertebrate_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
rm -f invertebrate_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt

cat *invertebrate_ftp* > invert_ftp_list.txt
cat invert_ftp_list.txt | while read line
do
	wget $line
done
...
if [ "$proj" == $proj ] ; then
	...
	mv -f *invertebrate_ftp_list* ~/$proj
	mv -f invert_ftp_list.txt ~/$proj
	cd $proj
