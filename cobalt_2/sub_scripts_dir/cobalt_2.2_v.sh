...
(Genbank = g, Refseq = r, continue = c)  " choice
	if [ "$choice" == "g" ] ; then
		...
		if [ "$choice2" == $choice2 ] ; then
			curl -o viral_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/viral/
			awk '{print $9}' viral_ftp_temp_list.html > viral_ftp_list_temp_1.txt
			grep -n $choice2 viral_ftp_list_temp_1.txt > viral_ftp_list_temp_2.txt
			cat viral_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > viral_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' viral_ftp_list_temp_3.txt > viral_ftp_list_temp_4.txt
			cat viral_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/viral/;g' > viral_ftp_list_temp_5.txt
			cat viral_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > viral_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat viral_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > viral_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat viral_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt viral_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > viral_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "r" ] ; then
		read -p "What Organism?  " choice2
		if [ "$choice2" == $choice2 ] ; then
			curl -o viral_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/viral/
			awk '{print $9}' viral_ftp_temp_list.html > viral_ftp_list_temp_1.txt
			grep -n $choice2 viral_ftp_list_temp_1.txt > viral_ftp_list_temp_2.txt
			cat viral_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > viral_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' viral_ftp_list_temp_3.txt > viral_ftp_list_temp_4.txt
			cat viral_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/viral/;g' > viral_ftp_list_temp_5.txt
			cat viral_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary_historical.txt;g' > viral_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat viral_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > viral_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat viral_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt viral_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > viral_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "c" ] ; then
	...
	if grep -q "$choice2" viral_ftp_list_$choice."$choice2".$(date +%m.%d.%Y%p)_3.txt ; then
...
rm -f viral_ftp_temp_list.html
rm -f viral_ftp_list_temp_1.txt
rm -f viral_ftp_list_temp_2.txt
rm -f viral_ftp_list_temp_3.txt
rm -f viral_ftp_list_temp_4.txt
rm -f viral_ftp_list_temp_5.txt
rm -f viral_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
rm -f viral_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt

cat *viral_ftp* > viral_ftp_list.txt
cat viral_ftp_list.txt | while read line
do
	wget $line
done
...
if [ "$proj" == $proj ] ; then
	...
	mv -f *viral_ftp_list* ~/$proj
	mv -f viral_ftp_list.txt ~/$proj
	cd $proj
