...
(Genbank = g, Refseq = r, continue = c)  " choice
	if [ "$choice" == "g" ] ; then
		...
		if [ "$choice2" == $choice2 ] ; then
			curl -o vert_mammal_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/vert_mammal/
			awk '{print $9}' vert_mammal_ftp_temp_list.html > vert_mammal_ftp_list_temp_1.txt
			grep -n $choice2 vert_mammal_ftp_list_temp_1.txt > vert_mammal_ftp_list_temp_2.txt
			cat vert_mammal_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > vert_mammal_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' vert_mammal_ftp_list_temp_3.txt > vert_mammal_ftp_list_temp_4.txt
			cat vert_mammal_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/vert_mammal/;g' > vert_mammal_ftp_list_temp_5.txt
			cat vert_mammal_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > vert_mammal_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat vert_mammal_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > vert_mammal_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat vert_mammal_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt vert_mammal_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > vert_mammal_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "r" ] ; then
		read -p "What Organism?  " choice2
		if [ "$choice2" == $choice2 ] ; then
			curl -o vert_mammal_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/vert_mammal/
			awk '{print $9}' vert_mammal_ftp_temp_list.html > vert_mammal_ftp_list_temp_1.txt
			grep -n $choice2 vert_mammal_ftp_list_temp_1.txt > vert_mammal_ftp_list_temp_2.txt
			cat vert_mammal_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > vert_mammal_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' vert_mammal_ftp_list_temp_3.txt > vert_mammal_ftp_list_temp_4.txt
			cat vert_mammal_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/vert_mammal/;g' > vert_mammal_ftp_list_temp_5.txt
			cat vert_mammal_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary_historical.txt;g' > vert_mammal_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat vert_mammal_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > vert_mammal_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat vert_mammal_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt vert_mammal_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > vert_mammal_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "c" ] ; then
	...
	if grep -q "$choice2" vert_mammal_ftp_list_$choice."$choice2".$(date +%m.%d.%Y%p)_3.txt ; then
...
rm -f vert_mammal_ftp_temp_list.html
rm -f vert_mammal_ftp_list_temp_1.txt
rm -f vert_mammal_ftp_list_temp_2.txt
rm -f vert_mammal_ftp_list_temp_3.txt
rm -f vert_mammal_ftp_list_temp_4.txt
rm -f vert_mammal_ftp_list_temp_5.txt
rm -f vert_mammal_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
rm -f vert_mammal_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt

cat *vert_mammal_ftp* > vm_ftp_list.txt
cat vm_ftp_list.txt | while read line
do
	wget $line
done
...
if [ "$proj" == $proj ] ; then
	...
	mv -f *vert_mammal_ftp_list* ~/$proj
	mv -f vm_ftp_list.txt ~/$proj
	cd $proj
