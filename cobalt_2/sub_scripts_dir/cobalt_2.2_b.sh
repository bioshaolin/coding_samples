...
(Genbank = g, Refseq = r, continue = c)  " choice
	if [ "$choice" == "g" ] ; then
		...
		if [ "$choice2" == $choice2 ] ; then
			curl -o bacteria_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/bacteria/
			awk '{print $9}' bacteria_ftp_temp_list.html > bacteria_ftp_list_temp_1.txt
			grep -n $choice2 bacteria_ftp_list_temp_1.txt > bacteria_ftp_list_temp_2.txt
			cat bacteria_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > bacteria_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' bacteria_ftp_list_temp_3.txt > bacteria_ftp_list_temp_4.txt
			cat bacteria_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/bacteria/;g' > bacteria_ftp_list_temp_5.txt
			cat bacteria_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > bacteria_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat bacteria_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > bacteria_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat bacteria_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt bacteria_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > bacteria_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "r" ] ; then
		read -p "What Organism?  " choice2
		if [ "$choice2" == $choice2 ] ; then
			curl -o bacteria_ftp_temp_list.html ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/bacteria/
			awk '{print $9}' bacteria_ftp_temp_list.html > bacteria_ftp_list_temp_1.txt
			grep -n $choice2 bacteria_ftp_list_temp_1.txt > bacteria_ftp_list_temp_2.txt
			cat bacteria_ftp_list_temp_2.txt | sed -e 's/:/\t/g' > bacteria_ftp_list_temp_3.txt
			awk -F "\t" '{print $2}' bacteria_ftp_list_temp_3.txt > bacteria_ftp_list_temp_4.txt
			cat bacteria_ftp_list_temp_4.txt | sed -e 's;^;ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/bacteria/;g' > bacteria_ftp_list_temp_5.txt
			cat bacteria_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary_historical.txt;g' > bacteria_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
			cat bacteria_ftp_list_temp_5.txt | sed -e 's;$;/assembly_summary.txt;g' > bacteria_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt
			cat bacteria_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt bacteria_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt > bacteria_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_3.txt
		fi
	elif [ "$choice" == "c" ] ; then
	...
	if grep -q "$choice2" bacteria_ftp_list_$choice."$choice2".$(date +%m.%d.%Y%p)_3.txt ; then
...
rm -f bacteria_ftp_temp_list.html
rm -f bacteria_ftp_list_temp_1.txt
rm -f bacteria_ftp_list_temp_2.txt
rm -f bacteria_ftp_list_temp_3.txt
rm -f bacteria_ftp_list_temp_4.txt
rm -f bacteria_ftp_list_temp_5.txt
rm -f bacteria_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p).txt
rm -f bacteria_ftp_list_$choice.$choice2.$(date +%m.%d.%Y%p)_2.txt

cat *bacteria_ftp* > bac_ftp_list.txt
cat bac_ftp_list.txt | while read line
do
	wget $line
done
...
if [ "$proj" == $proj ] ; then
	...
	mv -f *bacteria_ftp_list* ~/$proj
	mv -f bac_ftp_list.txt ~/$proj
	cd $proj
	...
	cat ftp_genome_list_temp2.txt | sed -e 's:^_genomic.fna.gz::g' > ftp_genome_list_temp3.txt
	cat ftp_genome_list_temp3.txt | sed -e 's:^bioproject_genomic.fna.gz::g' > ftp_genome_list.txt
	...
	rm -f ftp_genome_list_temp3.txt
	...
	cat ftp_gbff_list_temp2.txt | sed -e 's:^_genomic.gbff.gz::g' > ftp_gbff_list_temp3.txt
	cat ftp_gbff_list_temp3.txt | sed -e 's:^bioproject_genomic.gbff.gz::g' > ftp_gbff_list.txt
	...
	cat ftp_gff_list_temp2.txt | sed -e 's:^_genomic.gff.gz::g' > ftp_gff_list_temp3.txt
	cat ftp_gff_list_temp3.txt | sed -e 's:^bioproject_genomic.gff.gz::g' > ftp_gff_list.txt
	...
	cat ftp_rna_from_genomic_list_temp2.txt | sed -e 's:^_rna_from_genomic.fna.gz::g' > ftp_rna_from_genomic_list_temp3.txt
	cat ftp_rna_from_genomic_list_temp3.txt | sed -e 's:^bioproject_rna_from_genomic.fna.gz::g' > ftp_rna_from_genomic_list.txt
	...
	rm -f ftp_rna_from_genomic_list_temp3.txt
